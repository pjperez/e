  ...
  projects.forEach((p) => {
    ...
    pmore.addEventListener("click", (e) => {
      ...
      openRowMenu(pmore, row, [
        ...
        // The built-in project is the guaranteed home for chats, so it has no
        // Close entry at all rather than one that silently does nothing.
        ...(p.locked
          ? []
          : [{
              label: "Close",
...
async function refreshSessions(): Promise<void> {
  ...
  sbWs.title = ok ? ws : `Folder not found: ${ws} — open the project's ⋯ menu and rename it to pick another`;
}
